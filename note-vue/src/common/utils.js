...
export default {
    ...
    userLogin(username, password) {
        ...
        axios({
            ...
            if (result.data.code != 200) {
                this.floatMessage(result.data.message.toLowerCase(), 'error', 2000, true);
            } else {
    ...
    userRegister(username, password, randomsalt) {
        if (username == null || username.trim() == '' || password == null || password.trim() == '' || randomsalt == null || randomsalt.trim() == '') {
        ...
        MessageBox.confirm('要保存到数据库吗?', '提示', {
            ...
            .then(async function() {
                ...
                await axios
                    ...
                    .then((result) => {
                        if (result.data.code == 200) {
                            that.floatMessage('已经写进数据库啦，请放心', 'success', 2000, true);
                            // 保存成功后的一系列操作
